...
const BFS = (rp, bp) => {
  const queue = [[rp, bp, 0, ""]];
  while (queue.length) {
    let [rp, bp, cnt, path] = queue.shift();

    if (cnt === 10) return [-1];

    for (let [dx, dy, dir] of offset) {
      // 먼저 움직일 구슬 선택 (1 : r먼저, 0 : b먼저)
      ...
      if (arr[nrp[0]][nrp[1]] === "O") {
        return [cnt + 1, path + dir];
      }

      queue.push([nrp, nbp, cnt + 1, path + dir]);
    }
  }
  return [-1];
};
